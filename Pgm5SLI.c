...
int main(int argc, char *argv[]) {
    ...
    int numGPU;                                                              // Creates an integer to hold the number of Nvidia GPUs.
    int gpuID;                                                               // Creates an integer to hold the GPU ID. 
    ...
    numGPU = acc_get_num_devices(acc_device_nvidia);                         // Get the number of Nvidia GPUs if any, including multi-GPU support. 

    if(numGPU) {                                                             // If the number of Nvidia GPUs is greater than 0.
    
        gpuID = processID % numGPU;                                          // Assign each MPI process a GPU. 
        acc_set_device_num(gpuID, acc_device_nvidia);                        // Set the OpenACC to use the respective GPU. 
    }
 
    else {                                                                   // Otherwise, if there are no Nvidia GPUs available.
        
        acc_set_device_type(acc_device_host);                                // Set the OpenACC to the non-Nvidia GPU.
    }
    
    if (processID == 0) {                                                    // Checks if the process/node is the master node 0.
        printf("Nodes: %d, GPUs: %d\n", numProcessors, numGPU);              // Prints the Number of Processor nodes & the GPUs detetected for each run. 
    }
